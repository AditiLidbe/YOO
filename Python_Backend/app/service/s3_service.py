from uuid import uuid4

import boto3
from botocore.exceptions import BotoCoreError, ClientError
from fastapi import HTTPException, UploadFile, status

from app.utils.config import setting


def get_s3_client():
    if not setting.AWS_ACCESS_KEY_ID or not setting.AWS_SECRET_ACCESS_KEY:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="AWS credentials are not configured",
        )
    if not setting.AWS_S3_BUCKET_NAME:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="AWS bucket name is not configured",
        )

    return boto3.client(
        "s3",
        region_name=setting.AWS_REGION,
        aws_access_key_id=setting.AWS_ACCESS_KEY_ID,
        aws_secret_access_key=setting.AWS_SECRET_ACCESS_KEY,
    )


def upload_resume(file: UploadFile, user_id: int, job_id: int):
    client = get_s3_client()
    clean_file_name = file.filename.replace(" ", "_")
    file_path = f"users/{user_id}/jobs/{job_id}/resume/{uuid4()}_{clean_file_name}"

    try:
        client.upload_fileobj(
            file.file,
            setting.AWS_S3_BUCKET_NAME,
            file_path,
            ExtraArgs={"ContentType": file.content_type},
        )
    except (BotoCoreError, ClientError) as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not upload resume: {exc}",
        ) from exc

    return file_path


def get_download_url(file_path: str):
    client = get_s3_client()

    try:
        return client.generate_presigned_url(
            "get_object",
            Params={
                "Bucket": setting.AWS_S3_BUCKET_NAME,
                "Key": file_path,
            },
            ExpiresIn=3600,
        )
    except (BotoCoreError, ClientError) as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not create resume download URL: {exc}",
        ) from exc
